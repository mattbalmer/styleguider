...
function link(tree, parentpath) {
    console.log('linking', tree);

    tree.dependencies.forEach(function(dependency) {
        link(dependency, tree.path);
    });

    var linkPath = path.join(parentpath, '@'+tree.name),
        sourcePath = path.join(tree.cwd, tree.styleDir);
